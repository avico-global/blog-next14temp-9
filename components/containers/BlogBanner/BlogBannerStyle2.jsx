...
export default function BlogBannerStyle2({ myblog, imagePath }) {
  return (
    <div>
      <div className="flex flex-col items-start justify-center text-center gap-5 py-10">
        <Badge className=" text-white " >{myblog?.value?.article_category}</Badge>
        <h1
          style={{ fontSize: myblog?.value?.titleFontSize || 48 }}
          className="font-bold text-white capitalize max-w-screen-md"
        >
          {myblog?.value.title}
        </h1>

        <div className="flex text-gray-300 items-center justify-center gap-4">
